...
#################################################################
# Step 4 : Warp image based on src_points and dst_points
#################################################################
...
def detect_single(src_img, show_img, mtx, dist):
    # 1 Do RGB
    # distort_image = cv2.imread('150.jpg')
    # distort_image = cv2.imread('cam_test_images/100.jpg')
    distort_image = cv2.cvtColor(src_img, cv2.COLOR_BGR2RGB)
    # display(distort_image, 'distort_image')

    # 2 Do undistortion
    undistort_image = undistortImage(distort_image, mtx, dist)
    # display(undistort_image, 'undistort_image')

    # 3 Do sobel
    gray = cv2.cvtColor(undistort_image, cv2.COLOR_RGB2GRAY)
    sobel = directional_gradient(gray, direction='x', thresh=[30, 255])
    # display(sobel, 'Gradient x', color=0)
    # 4 Do hls
    # hls = color_binary(undistort_image, dst_format='HLS', ch=3, ch_thresh=[50,255])
    hls = white_select(undistort_image, thresh=[80, 155])
    # display(hls, 'HLS to Binary S', color=0)
    # 5 Do Combined
    combined_binary = np.zeros_like(sobel)
    combined_binary[((sobel == 1) | (hls == 1))] = 1
    # display(combined_binary, 'Combined output', color=0)
    ...
    show_img = draw_area(show_img, combined_binary, transform_matrix['Minv'], left_fit, right_fit, flag)
    distance_from_center = calculate_curv_and_pos(warped_image, left_fit, right_fit, flag)
    draw_values(show_img, distance_from_center)
    return show_img
...
def _main():
    ...
    # data
    image_path = './cam_test_images/'
    image_save_path = './save_test_images/'
    ...
    for image_name in os.listdir(image_path):
        ...
        distort_image = image
        # 1 Do RGB
        distort_image = cv2.cvtColor(distort_image, cv2.COLOR_BGR2RGB)
        ...
        # 9 draw
        result = draw_area(undistort_image, combined_binary, transform_matrix['Minv'], left_fit, right_fit, flag)

        # 10 calcurate bias
        dist_from_center = calculate_curv_and_pos(warped_image, left_fit, right_fit, flag)
        dist_center.append(dist_from_center)
        draw_values(result, dist_from_center)
        cv2.imwrite(os.path.join(image_save_path, image_name), result)
